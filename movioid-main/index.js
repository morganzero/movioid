...
const { WHMCS_CLIENT_ID, WHMCS_CLIENT_SECRET, LINKED_ROLE_ID, JWT_SECRET, DISCORD_TOKEN } = process.env;
...
app.get(
  "/callback",
  (req, _res, next) => {
    jwt.verify(req.query.state.split(" ")[1], JWT_SECRET);
    next();
  },
  async (req, res) => {
    const key = jwt.verify(req.query.state.split(" ")[1], JWT_SECRET);
    if (!req.query.code) {
      res.status(400).json({
        success: false,
